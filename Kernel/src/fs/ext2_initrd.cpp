...
read_initrd(
	...
	if (start_block == end_block) {
		memcpy(buffer, (void *)((void*)ext2_get_inode_block(inode, start_block) + offset % block_size), size_to_read);
		return size_to_read;
	} else {
		...
		for (block_offset = start_block; block_offset < end_block; ++block_offset) {
			if (block_offset == start_block) {
				memcpy(buffer, (void *)((void*)ext2_get_inode_block(inode, block_offset) + (offset % block_size)), (block_size - (offset % block_size)));
			} else {
...
finddir_initrd(
	...
	while (dir_offset < inode->size) {
		ext2_dir_t * d_ent = (ext2_dir_t *)((void*)block + dir_offset);
#if 0
	...
	fs_node_t * outnode = (fs_node_t*)malloc(sizeof(fs_node_t));
...
initrd_node_from_file(
	...
	fnode->read    = (read_type_t)read_initrd;
...
ext2_get_inode(
	...
	return (ext2_inodetable_t *)((void*)initrd_inode_table + initrd_superblock->inode_size * (inode - 1));
}
...
ext2_get_inode_block(
	...
	if (block < 12) {
		...
		return ext2_get_block(*(uint32_t*)((void*)ext2_get_block(inode->block[12]) + (block - 12) * sizeof(uint32_t)));
	} else if (block < 12 + 256 + 256 * 256) {
		uint32_t a = block - 12;
		uint32_t b = a - 256;
		uint32_t c = b / 256;
		uint32_t d = b - c * 256;
		void * c_block = ext2_get_block(*(uint32_t *)((void*)ext2_get_block(inode->block[13]) + (c) * sizeof(uint32_t)));
		return ext2_get_block(*(uint32_t *)((void*)c_block + (d) * sizeof(uint32_t)));
	}
	return nullptr;
}
...
ext2_get_block(
	...
	return (void *)((void*)initrd_start + (1024 << initrd_superblock->log_block_size) * block);
}
...
initrd_mount(
	...
	initrd_root_block = (ext2_bgdescriptor_t *)((void*)initrd_start + 1024 + 1024);
	initrd_inode_table = (ext2_inodetable_t *)((void*)initrd_start + (1024 << initrd_superblock->log_block_size) * initrd_root_block->inode_table);
	ext2_inodetable_t * root_inode = ext2_get_inode(2);
	initrd_root = (fs_node_t *)malloc(sizeof(fs_node_t));
	//initrd_node_root(root_inode, initrd_root);
